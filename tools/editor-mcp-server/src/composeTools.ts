...
export function createComposeToolRunner(deps: ComposeToolDeps) {
  ...
  const latestComposition = (engine: SessionEngine): CompositionManifest | null => {
    for (let index = engine.session.steps.length - 1; index >= 0; index -= 1) {
      const step = engine.session.steps[index]!
      const composition = (step.result as { composition?: CompositionManifest } | undefined)?.composition
      if (step.kind === 'compose:game' && step.status === 'completed' && composition) return composition
    }
    return null
  }

  const assembleEvidence = async (gameId: string, spec: GameSpec, engine: SessionEngine): Promise<SliceEvidence | ToolResult> => {
    const composition = latestComposition(engine)
    if (!composition) return fail(`no compose:game step for "${gameId}" — call composeGame first`)
    const gates: SliceGateResult[] = GATES.map(({ kind, step }) => {
      ...
      if (kind === 'evaluate') {
        const outcome = (record.result as { outcome?: string } | undefined)?.outcome
        return { kind, status: outcome === 'passed' ? 'passed' : 'failed', stepId: record.id }
      }
    ...
    return {
      gameId, specVersion: spec.specVersion, specHash: hashJson(spec), compositionHash: hashJson(composition),
  ...
  return {
    async execute(name: string, raw: unknown): Promise<ToolResult> {
      if (name === 'composeGame') {
        ...
        return ok({ ...output.summary, compositionHash, files: output.files.map((file) => file.path), cached: guarded.cached, stepId: guarded.step.id, sliceCheckpoint: sliceCheckpointStatus(found.engine, { specHash, compositionHash }) })
      }
