...
describe('Runner', () => {
  ...
  it('runs the browser smoke once then serves it from the dist-fingerprint cache', async () => {
    ...
    const store = await openSessionStore(root, { now: () => 1 })
    const runner = createRunner({ repoRoot: root, gameId: 'g', store, snapshot, exec: okExec, browserSmoke: smoke, evaluate, now: () => 1 })
    expect((await runner.run('browser', false)).ok).toBe(true)
    expect((await runner.run('browser', false)).content).toMatchObject({ skipped: 'cached' })
    expect(smoke).toHaveBeenCalledTimes(1)
    await store.release()
  })

  it('records a browser smoke failure as a finding, not a throw', async () => {
    const root = await gameRepo()
    const failing = async () => ({ booted: false, consoleErrors: ['boom'], frameMs: [], screenshotPath: null })
    const store = await openSessionStore(root, { now: () => 1 })
    const runner = createRunner({ repoRoot: root, gameId: 'g', store, snapshot, exec: okExec, browserSmoke: failing, evaluate, now: () => 1 })
    const result = await runner.run('browser', false)
    expect(result.ok).toBe(false)
    expect(store.state.findings.some((f) => f.step === 'browser' && f.severity === 'error')).toBe(true)
    await store.release()
  })

  it('caches evaluate on identical snapshot+options and reports freshness from stored options', async () => {
    const root = await gameRepo()
    const evalFn = vi.fn(evaluate)
    const store = await openSessionStore(root, { now: () => 1 })
    const runner = createRunner({ repoRoot: root, gameId: 'g', store, snapshot, exec: okExec, browserSmoke: browser, evaluate: evalFn, now: () => 1 })
    expect((await runner.run('evaluate', false, { maxSteps: 10 })).ok).toBe(true)
    expect(await runner.freshness('evaluate')).toBe('fresh')
    await runner.run('evaluate', false, { maxSteps: 10 })
    expect(evalFn).toHaveBeenCalledTimes(1)
    await store.release()
  })

  it('records an evaluate failure as a finding', async () => {
    const root = await gameRepo()
    const failing = async () => ({ ok: false, content: { error: 'bad' } })
    const store = await openSessionStore(root, { now: () => 1 })
    const runner = createRunner({ repoRoot: root, gameId: 'g', store, snapshot, exec: okExec, browserSmoke: browser, evaluate: failing, now: () => 1 })
    const result = await runner.run('evaluate', false, { maxSteps: 10 })
    expect(result.ok).toBe(false)
    expect(store.state.findings.some((f) => f.step === 'evaluate' && f.severity === 'error')).toBe(true)
    await store.release()
  })
})
