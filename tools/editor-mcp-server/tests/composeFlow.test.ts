...
describe('Phase 3 exit criterion — spec → compose → evaluate → slice checkpoint', () => {
  ...
  it('requires a current approved composition before rendering slice evidence', async () => {
    const { host } = await setup()
    await host.executeTool('compileGameSpec', { gameId: 'probe', draft: sliceDraft('probe'), prompt: 'slice', translations: [] })
    await host.executeTool('renderDesignBrief', { gameId: 'probe' })
  ...
  it('marks old checks stale after composing the current spec', async () => {
    const { host } = await setup()
    await host.executeTool('compileGameSpec', { gameId: 'probe', draft: sliceDraft('probe'), prompt: 'slice', translations: [] })
    await host.executeTool('renderDesignBrief', { gameId: 'probe' })
    await host.executeTool('recordDesignDecision', { gameId: 'probe', decision: 'approve', reason: 'v1' })
    await host.executeTool('composeGame', { gameId: 'probe' })
    for (const tool of ['runBuild', 'runTests', 'runBrowserEval']) await host.executeTool(tool, { gameId: 'probe' })
    await host.executeTool('openProject', { gameId: 'probe' }); await host.executeTool('evaluate', {})
    const draft2 = sliceDraft('probe'); (draft2.identity as Record<string, unknown>).logline = 'Version two.'
    await host.executeTool('compileGameSpec', { gameId: 'probe', draft: draft2, prompt: 'slice', translations: [], changeReason: 'v2' })
    await host.executeTool('renderDesignBrief', { gameId: 'probe' })
  ...
  it('requires current gate provenance and a full test run', async () => {
    const { host } = await setup()
    await host.executeTool('compileGameSpec', { gameId: 'probe', draft: sliceDraft('probe'), prompt: 'slice', translations: [] })
    await host.executeTool('renderDesignBrief', { gameId: 'probe' })
    await host.executeTool('recordDesignDecision', { gameId: 'probe', decision: 'approve', reason: 'go' })
    await host.executeTool('composeGame', { gameId: 'probe' })
    await host.executeTool('runBuild', { gameId: 'probe' })
    await host.executeTool('runTests', { gameId: 'probe', scope: 'one case' })
    await host.executeTool('runBrowserEval', { gameId: 'probe' })
    await host.executeTool('openProject', { gameId: 'probe' }); await host.executeTool('evaluate', {})
    const report = await host.executeTool('renderSliceReport', { gameId: 'probe' })
    const testGate = (report.content as { gates: Array<{ kind: string; status: string }> }).gates.find((gate) => gate.kind === 'test')
    expect(testGate?.status).toBe('stale')
    await host.dispose()
  })
})
