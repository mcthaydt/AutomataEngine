...
function playingCtx(): { ctx: GameCtx; store: GameStore; feedback: EventQueue } {
  ...
  return { ctx, store, feedback: new EventQueue() }
}
...
describe('goal', () => {
  ...
  it('emits a "goalReached" feedback fact at the goal position', () => {
    const events = new EventQueue()
    events.emit({
      type: 'sensorEnter',
      a: { ball: {} } as Entity,
      b: { goal: {}, transform: createTransform({ x: 0, y: 0, z: -6 }) } as Entity
    })
    const { ctx, feedback } = playingCtx()
    createGoal(events, feedback).run(ctx)
    expect(feedback.read<FeedbackEvent>('feedback')).toEqual([
      { type: 'feedback', kind: 'goalReached', origin: { x: 0, y: 0, z: -6 } }
    ])
  })
