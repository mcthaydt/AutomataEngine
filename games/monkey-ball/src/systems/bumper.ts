...
export function createBumper(physics: PhysicsPort, events: EventQueue, feedback: EventQueue): System<GameCtx> {
  return {
    name: 'bumper',
    stage: 'postPhysics',
    run(ctx) {
      ...
      for (const event of events.read<PairEvent>('contactStart')) {
        ...
        emitFeedback(feedback, 'bumped', bumper.transform.position)
      }
    }
  }
}
