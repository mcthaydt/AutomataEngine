...
- [x] **Step 3: Implement — add the M8 systems to `createGameplay`**
...
- [x] **Step 4: Run the M8 + M7 gameplay tests**
...
Review patch: `gameplay-m8.test.ts` also asserts the runner wiring for bumper
impulses, moving-platform kinematic targets, and camera `setCamera` calls, so
removing those system registrations fails the integration test.
