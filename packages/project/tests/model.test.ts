...
const snapshot = {
  ...
  resources: {
    tuning: { id: 'tuning', typeId: 'fake.tuning', data: { speed: 4 } }
  }
}

describe('projectSnapshotSchema', () => {
  it('accepts a current project snapshot without erasing component/resource data', () => {
    expect(projectSnapshotSchema.parse(snapshot)).toEqual(snapshot)
  })

  it.each([
    [{ ...snapshot, manifest: { ...snapshot.manifest, formatVersion: 1 } }, 'formatVersion'],
